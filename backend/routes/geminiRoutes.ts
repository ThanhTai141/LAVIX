...
router.post('/chat', protect, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
router.post('/chat-with-file', protect, validateFileUpload, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { message } = req.body;
    const userId = req.user?._id;
    const file = req.file;
...
// Route tạo chat mới
router.post('/new-chat', protect, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
...
router.get('/history', protect, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?._id;
