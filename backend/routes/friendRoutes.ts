...
// Gửi lời mời kết bạn
router.post('/request', protect, async (req, res): Promise<void> => {
...
// Chấp nhận lời mời
router.post('/accept', protect, async (req, res): Promise<void> => {
  const { user } = req as AuthenticatedRequest;
...
// Từ chối lời mời
router.post('/reject', protect, async (req, res): Promise<void> => {
  const { user } = req as AuthenticatedRequest;
  const userId = user._id;
  const { requestId } = req.body;
  const request = await FriendRequest.findById(requestId);
...
// Hủy lời mời đã gửi
router.post('/cancel', protect, async (req, res): Promise<void> => {
  const { user } = req as AuthenticatedRequest;
  const userId = user._id;
  const { requestId } = req.body;
  const request = await FriendRequest.findById(requestId);
...
// Danh sách bạn bè
router.get('/list', protect, async (req, res): Promise<void> => {
  const { user } = req as AuthenticatedRequest;
  const userId = user._id;
...
router.get('/', protect, async (req, res): Promise<void> => {
  const { user } = req as AuthenticatedRequest;
  const userId = user._id;
  const userDoc = await User.findById(userId).populate('friends', 'fullName email avatar isOnline lastSeen');
  res.json({ friends: userDoc?.friends || [] });
